...
module "k8s_role_udm" {
  ...
  k8s_bound_service_account_namespaces = ["omec"]
  k8s_token_policies                   = ["udm-policy"]
...
module "k8s_role_webconsole" {
  ...
  k8s_backend                          = module.k8s_auth_dev.auth_backend_path
  k8s_role_name                        = "webconsole"
  k8s_bound_service_account_names      = ["webconsole"]
  k8s_bound_service_account_namespaces = ["omec"]
  k8s_token_policies                   = ["webconsole-policy"]
  k8s_token_ttl                        = 3600  # 1 hour
}
